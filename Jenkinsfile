pipeline {
    ...
    stages {
        ...
        stage('Store Maven Artifact') {
            ...
            steps {
                sh 'env'
                sh "curl -v -u $MAVEN_USER:$MAVEN_PASS --upload-file java_webapp/target/java-webapp*.jar http://nexus-internal.testcompany.shipyard.cloud:80/repository/maven-releases/com/my/group/myArtifact/1.0.0-RC1/java-webapp.jar"
            }
        }
    }
}