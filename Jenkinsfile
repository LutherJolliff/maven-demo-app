        ...
        stage('Sonarqube Analysis') {
            ...
            steps {
                withSonarQubeEnv('Cynerge Sonarqube') {
                    ...
                    sh "${scannerHome}/bin/sonar-scanner -Dsonar.login=$SONAR_TOKEN -Dsonar.projectKey=$SONAR_PROJECT -Dsonar.tests=. -Dsonar.junit.reportPaths=$SONAR_REPORTS -Dsonar.coverage.jacoco.xmlReportPaths=$JACOCO_REPORT"
                }
            }
        }
    }
